...
def generate_qp_samples(Q, P, n=2, m=1, num_samples=10000):
    ...
    while len(results) < num_samples:
        ...
        results.append((x_opt.tolist(), A[0, 1], b[0]))

    df_results = pd.DataFrame(
        results, columns=["x", "A_perturbed_01", "b_perturbed_0"]
    )
    df_variables = pd.DataFrame(
        variables, columns=["A_base", "b_base"]
    )
    return df_results, A_base, b_base
...
# Q = -Q.T @ Q 
Q = np.array([[2, 0], [0, 3]])
P = np.array([1, 1]) # np.random.rand(2)
df, A_base, b_base = generate_qp_samples(
    Q, P, n=2, m=1, num_samples=10000
)
...
df_save.to_csv('./data/train_data_qp.csv', index=False)
...
df_val.to_csv('./data/val_data_qp.csv', index=False)
...
df_test.to_csv('./data/test_data_qp.csv', index=False)
...
df_test_vars.to_csv('./data/qp_base_variables.csv', index=False)
