...
input_file="hyperparams_sampling.txt" #"hyperparams_local.txt"

while read -r a b c d e f g h i j; do
    ...
    submission_script="submit/submit_${j}.sh"
    cat > "$submission_script" <<EOT
#!/bin/bash
#SBATCH --job-name=submit/myjob_${j}
#SBATCH --error=error/myjob_${j}.err
#SBATCH --nodes=1
...
#$conda_env_path/bin/python3 plot_sampling_step_by_step.py --n_layers $a --hidden_units $b --optimizer $c --conditioning_type $d --activation $e --batch_size $f --lr $g --seed $h  --id $i --eps $j --annealed_step $k --id_script $l > "$output_file" 

$conda_env_path/bin/python3 plot_sampling_step_by_step.py --n_layers $a --hidden_units $b --conditioning_type $c --activation $d --seed $e  --eps $f --id $g --annealed_step $h --project $i --id_script $j > "$output_file" 
